...
// データディレクトリのパスを取得
// 配布版：exe の隣の data/ フォルダ（難読化対象外）
// 開発版：PROJECT_ROOT の data/ フォルダ
const getDataDir = () => {
    try {
        const { app } = require('electron');
        console.log('🔍 [set-manager] app.isPackaged:', app.isPackaged);
        if (app && app.isPackaged) {
            // 配布版：exe の隣の data/ フォルダ
            const exePath = process.execPath; // .../Prompt Classifier v3.exe
            const exeDir = path.dirname(exePath); // .../Prompt Classifier v3-win32-x64/
            const dataPath = path.join(exeDir, 'data');
            console.log('🔍 [set-manager] 配布版 exePath:', exePath);
            console.log('🔍 [set-manager] 配布版 exeDir:', exeDir);
            console.log('🔍 [set-manager] 配布版 dataPath:', dataPath);
            return dataPath;
        }
    } catch (e) {
        // Electronが利用できない環境（テストなど）
        console.warn('⚠️ [set-manager] Electron require失敗:', e.message);
    }
    // 開発版：build/data/ フォルダ（ビルド対象外）
    const devDataPath = path.join(PROJECT_ROOT, 'build', 'data');
    console.log('🔍 [set-manager] 開発版 dataPath:', devDataPath);
    return devDataPath;
};
